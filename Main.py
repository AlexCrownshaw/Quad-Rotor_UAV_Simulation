...
from Config_JSON.Config_Load_Functions import load_vehicle_properties, load_flight_path, load_sensor_config, load_disturbances
...
from Simulation.Disturbance import Disturbance
...
DISTURBANCE_JSON_PATH = r"Config_JSON/disturbances.json"
...
def main():
    ...
    disturbances = load_disturbances(DISTURBANCE_JSON_PATH)
    ...
    run_simulation(dynamics, thrust, control, sensors, estimation, X, G, dynamics_data, disturbance)
...
def run_simulation(dynamics: DynamicsModel, thrust: ThrustModel, control: ControlSystem, sensors: Sensors,
    ...
    for t in np.arange(t_delta, t_duration + t_delta, t_delta):
        ...
        D = disturbance.get_disturbance(t)
        X, X_dot = dynamics.rk4(X, T, Q, D, t_delta)
